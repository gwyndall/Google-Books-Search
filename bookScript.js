	...
	$('#add-btn').on('click', function (event) {
		...
		function searchBook() {
			...
			if (isbn) {
				...
				if (!title) {
					var queryURL = 'https://www.googleapis.com/books/v1/volumes?q=inauthor:' + author;
				} else if (!author) {
					var queryURL = 'https://www.googleapis.com/books/v1/volumes?q=intitle:' + title;
				} else {
					var queryURL =
			...
			$.ajax({
				...
				if (response.totalItems == 0) {
					...
					for (let i = 0; i < response.items.length; i++) {
						...
						var bookView = $("<button type='button' class='view-book' data-id='" + [i] + "'>")
							// .attr('href', '')
							.text('View');
